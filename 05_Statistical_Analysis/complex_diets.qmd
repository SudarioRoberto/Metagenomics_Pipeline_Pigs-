...
Este relatório está dividido em duas partes. A primeira contém toda a análise estatística dos dados metagenômicos. Vale lembrar que todo o processamento bioinformático já foi realizado, incluindo: remoção de reads de baixa qualidade, remoção de contaminação do hospedeiro, classificação taxonômica com Kraken2 e cálculo da abundância absoluta e relativa de reads para cada unidade taxonômica. Agora vamos carregar os arquivos de abundância e os metadados no R.
...
A diversidade alfa é calculada sobre a abundância relativa. Se você somar a abundância relativa de todas as espécies dentro de uma amostra, o total deve ser igual ou muito próximo de 1. A diversidade alfa possui várias métricas - aqui utilizamos as principais. Importante: a métrica Chao1 deve ser calculada sobre o número de contagens (counts) e não sobre a abundância relativa. Por isso, observe que as tabelas têm nomes diferentes: \`\_frac\` para abundância relativa e \`\_counts\` para contagens absolutas.
...
Ao comparar os tratamentos usando ANOVA, observamos diferenças significativas, principalmente nos índices de Shannon e Simpson, onde o grupo com protease e dieta complexa apresentou maior diversidade. Porém, quando utilizamos testes não-paramétricos com correção para taxa de falsas descobertas (FDR), os resultados não são significativos. Do ponto de vista estatístico conservador, a conclusão seria que não há diferença significativa. No entanto, a interpretação final depende do contexto biológico e da comparação com outras variáveis do estudo (imunologia, desempenho, etc.).
...
Novamente, os resultados não são significativos após correção para FDR, embora sejam significativos considerando apenas o p-valor não ajustado. Esta discrepância é comum em estudos com tamanho amostral limitado e deve ser interpretada com cautela.
...
Nesta seção utilizamos o ANCOM-BC, outro método para identificar diferenças na abundância entre espécies. Este método também não encontrou resultados significativos após ajuste para FDR. No entanto, no heatmap final incluímos os táxons que apresentaram p-valor significativo (antes da correção) para a interação entre os fatores. Vale a pena explorar esses resultados para identificar táxons de interesse biológico para o estudo.
...
A abordagem de consenso identifica táxons que foram significativos (p \< 0.05) em ambos os métodos (ALDEx2 e ANCOM-BC). Esta estratégia aumenta a confiabilidade dos resultados, reduzindo falsos positivos.
...
Esta seção permite criar gráficos de boxplot para táxons específicos de interesse. Para alterar os táxons visualizados, basta modificar os nomes na variável \`selected_taxa\`. O código abaixo mostra como buscar nomes de táxons disponíveis nos dados. Os gráficos são gerados separadamente para cada fator (Complex e Protease).
...
Esta seção gera gráficos mostrando a interação entre os fatores Complex e Protease para táxons específicos. Modifique a variável \`selected_taxa\` para visualizar os táxons de seu interesse.
